...
// Radix build of the footer-aggregation shim. Keep the EXPORTED API identical
// to its base-ui twin, footer-aggregation.tsx — only FooterMethodPicker's
// trigger composition differs: the base-ui build composes via base-ui's
// `render` prop; this build uses Radix's `asChild` with a nested <button>
// (the same divergence primitives.tsx/primitives.radix.tsx already
// establishes for PopoverButtonTrigger). This file is distribution-only: it
// is excluded from this repo's typecheck (tsconfig `**/*.radix.tsx`) and is
// never imported here — it is validated against real Radix primitives in a
// consumer project.
...
/** Radix build of the footer-aggregation shim — see footer-aggregation.tsx for the base-ui build. */
