...
**No deviations.** Implementer's plan-assumed file shapes matched the real files exactly (fixture `DATA`, `columns()`, the `DataTableColumnMeta` import line, the `</TableBody>`/`</Table>` structure) — no adaptation needed. Spec review independently re-verified all 5 wiring points + both new tests + hand-recomputed the expected sum (74) against the real fixture; 147/147 regression, lint/typecheck clean. Code-quality review found nothing blocking: the `useFooterAggregation` call's 6 named args (vs. `useDataTable(props)`'s spread) is a legitimate API-shape difference, not sloppiness; `computeAggregate`-omitted-while-`calculableColumns`-set was traced and confirmed to gracefully degrade to client-side aggregation with a `(loaded rows)` partial-qualifier, per Task 5's existing design. One cross-cutting gap noted for awareness, not fixed here (out of this task's scope): `footer-aggregation.tsx`'s `FooterValueCell` (a Task 6 file, unmodified by Task 7) renders plain `TableCell`s with no sticky/pinned-column treatment, unlike header/body cells — worth checking during Task 11's final review whether a pinned column's footer cell visibly misaligns on horizontal scroll. Commit `8b1fcb6`.
...
**No deviations.** Purely mechanical re-export, matched the plan's given code verbatim including the guessed `use-footer-aggregation.ts` type names (`UseFooterAggregationOptions`/`FooterAggregationResult`, both confirmed real). Spec review cross-checked every export against its source module (no typos, nothing fabricated) and confirmed exactly one `./types` type-export block. Code-quality review confirmed sensible dependency-order grouping and, cross-checking against Tasks 1-7's actual shipped surface, found nothing consumer-facing left un-exported. 147/147 regression, lint/typecheck clean. Commit `9e3156c`.
...
**No deviations.** All 4 new files (`aggregate.ts` as `registry:lib`, `row-gutter.tsx`/`footer-aggregation.tsx` as `registry:component`, `use-footer-aggregation.ts` as `registry:hook`) added identically to both `data-table` and `data-table-radix`, each interleaved next to its closest type-sibling rather than block-appended. Confirmed `components/ui/table.tsx` needed no registry change — it's never a direct file entry, only pulled in via the `"table"` string in `registryDependencies`. Spec review independently rebuilt from scratch and confirmed byte-identical output, no duplicates, other registry items (`grouped-data-table`, `table-fields`) untouched. Code-quality review found the placement genuinely better than the plan's minimum bar (grouped by type-sibling, not just appended) and formatting fully consistent. Commit `13aa612`.
...
**No deviations in the wiring itself** (`hoursLogged`/`budget` matched the plan's assumed column ids exactly). **Real bug found during Step 2's manual browser verification** (not by a subagent — caught live, by the user, hovering the demo in a real browser): a row's checkbox, once revealed by hovering, stayed stuck visible after the mouse moved away. Root-caused empirically (via `getComputedStyle`/`getBoundingClientRect` inspection in a live browser, not guessed) to the shared `Checkbox` primitive's own oversized invisible touch-target (`after:-inset-x-3 after:-inset-y-2` in `components/ui/checkbox.tsx`) spilling a few pixels past the tiny gutter cell into the neighboring row, so the pointer could cross rows without ever firing the original row's `mouseleave`. Fixed by rewriting `RowGutterCell` to drop the `onMouseEnter`/`onMouseLeave`/`onFocus`/`onBlur` React state entirely in favor of a CSS-only `[tr:hover_&]`/`group-focus-within` reveal — both the number and checkbox are always mounted, only visibility toggles, driven by real-time pseudo-classes that can't get "stuck" the way discrete DOM events can. Verified the exact adjacent-row hover transition that broke before, plus keyboard focus/blur reveal, directly in the browser post-fix. Also found and fixed an adjacent dormant bug while investigating: `data-state="selected"` was never actually being set on body `<TableRow>`s, so both the row's own native selected-background (`data-[state=selected]:bg-muted`, already present in `components/ui/table.tsx`) and Task 7's pinned-cell `[tr[data-state=selected]_&]` selector were silently dead code — fixed in `data-table.tsx`. Additionally implemented shift-click range selection (`DataTableRuntime.toggleRowSelected`) per a direct user request during this same browser-verification session — Sheets/Gmail-style: a plain click sets the anchor row, a shift-click extends an inclusive range to it. **A dispatched code-quality review of that fix then caught a critical follow-on bug**: the shift-click feature (and, once traced further, the pre-existing Task 2 row-number display too) used TanStack's `row.index`, which is fixed at row creation to the row's position in the *original, unsorted* `data` array — `getSortedRowModel` preserves it via a shallow `{...row}` copy rather than reassigning it — so it silently pointed at the wrong row (or no-op'd) under sorting, for both `toggleRowSelected` and the row-gutter's own displayed row number. Reproduced live (sorting by Budget scrambled the row-number gutter, e.g. `3, 10, 2, 12...` instead of `1, 2, 3...`). Fixed by switching `toggleRowSelected` to an id-based anchor with a live `findIndex` lookup against the current row model, and the row-number display to `table.getRowModel().rows.findIndex(r => r.id === row.id)`. That second fix attempt initially used `.indexOf(row)` (reference equality) instead of `findIndex` by id, which reproduced a DIFFERENT bug (all row numbers showing `0`) — root-caused live in the browser via a React-fiber inspection showing `table.getRowModel().rows[i] !== row` for the exact same logical row (same id, different object instance), confirming TanStack's row objects aren't reference-stable across `getRowModel()` calls. Both the isolated `row-gutter.test.tsx` mocks and a new full `<DataTable>` integration test (`data-table.test.tsx`, sorts real data and asserts on real rendered row numbers + shift-click selection) were updated/added to catch this class of bug — the original mock tests had passed despite the bug because they placed the exact same row reference into their mock row model, unlike a real table. Re-verified end-to-end in the browser post-fix: sorting no longer scrambles row numbers, and shift-click range-selects by true on-screen position. Full suite: 236/236. Typecheck/lint clean. Commit `ab5a6dc` (amended).
...
**Step 1-2 results:** 233/233 (later 240/240 after the fixes below), typecheck/lint/build all clean. **Step 3 (manual browser smoke test)** covered row selection via checkbox click/hover/header tri-state, footer method picker + selection-aware scope switching, sorting, and keyboard nav (arrow-left from the leftmost real column correctly stays put rather than entering the gutter column) — all confirmed working, PLUS surfaced and fixed 3 real bugs along the way (see the commit history: the hover-stuck-checkbox bug and its `row.index`/reference-equality follow-ons, and the Columns menu leaking `__row-gutter__` as visible text).

**Step 4 (final holistic review) found 4 more real, cross-file bugs** the per-task reviews structurally could not have caught, since each task's review only sees that task's own files/spec:
1. **Critical, Radix-only crash:** `RowGutterCell`'s checkbox `onCheckedChange` read `eventDetails.event.shiftKey`, assuming base-ui's `(checked, eventDetails) => void` signature — but Radix's own `Checkbox.Root.onCheckedChange` is `(checked) => void`, no second argument at all (radix-ui/primitives#734). Since `row-gutter.tsx` ships unmodified to the `data-table-radix` registry item (no `.radix.tsx` twin existed), every row-checkbox click would throw for a Radix consumer. Fixed by capturing `shiftKey` via `onPointerDown`/`onKeyDown` instead — a plain DOM prop both primitives forward identically, so no twin is needed for this part.
2. **Critical, Radix-only broken tri-state:** `SelectAllHeader` passed a separate boolean `indeterminate` prop to `Checkbox` (base-ui's own API) — Radix has no such prop; its mixed state is expressed by passing the *string* `"indeterminate"` as `checked` itself. This part genuinely can't be made cross-compatible with one prop-passing incantation, so it required an actual twin: created `row-gutter.radix.tsx` (matching the established `primitives.tsx`/`primitives.radix.tsx` split), differing only in this one line (`checked={checked ? true : indeterminate ? "indeterminate" : false}`), and rewired `registry.json`'s `data-table-radix` item to point at it (`pnpm registry:build` re-run, verified the built `data-table.json`/`data-table-radix.json` each resolve to the correct build's content under the target filename `row-gutter.tsx`).
3. **Important, Radix styling/a11y regression:** `FooterMethodPicker`'s Popover trigger used base-ui's `render` prop directly instead of routing through the project's own `PopoverButtonTrigger` shim (or an equivalent) — Radix has no `render` prop (it uses `asChild`), so the trigger's `aria-label`/`type`/custom classes would silently never reach the real Radix consumer's rendered button. Fixed the same way as #2: created `footer-aggregation.radix.tsx`, differing only in `FooterMethodPicker`'s trigger composition (`asChild` + nested `<button>`), wired into `registry.json`.
4. **Important, state-consistency bug:** `isAllMatchingSelected` never got reconciled when the `data` prop changed and new rows appeared (e.g. a new server page loading under `manualPagination`) — the header kept asserting "all matching selected" while the newly-loaded rows' own checkboxes rendered unselected (their ids were never in `rowSelection`), a directly visible, self-contradictory UI state. Fixed with a `React.useEffect` in `use-data-table.ts` that re-applies `table.toggleAllRowsSelected(true)` whenever `rowIds` changes while `isAllMatchingSelected` is true, mirroring `setAllMatchingSelected`'s own existing "turning matching ON also selects every loaded row" behavior.

All four re-verified: full suite (240/240), typecheck/lint/build clean, and live in the browser (shift-click + footer recalculation confirmed correct post-refactor; built `data-table-radix.json`'s content independently inspected via `node -e` to confirm it contains the Radix-specific code, not the base-ui version). Given the severity, dispatched a final re-review of the fix itself (same discipline as the earlier `row.index` fix): confirmed both `.radix.tsx` twins are byte-identical to their base-ui counterparts apart from the documented one-line divergences; confirmed Radix's `Checkbox.Root` genuinely accepts `checked: boolean | "indeterminate"` and that `checked`/`indeterminate` are mutually exclusive by construction in `SelectAllHeader` (`indeterminate = !checked && ...`), so the ternary can't silently pick the wrong branch; confirmed `shiftKeyRef` (a stable `useRef`) has no staleness risk between pointerdown and click, and `onKeyDown` correctly covers Space/Enter-triggered checkbox activation; confirmed `registry.json`'s wiring is complete (no leftover plain-`.tsx` references for these two components in the radix item, non-radix item unchanged) and the built `public/r/*.json` genuinely differ; confirmed the `isAllMatchingSelected` reconciliation effect has no infinite-loop risk (`rowIds` depends only on `data`/`columns`, never on `rowSelection`) and doesn't fire in the normal non-all-matching case. One nice-to-have found (a comment in `row-gutter.tsx` that became stale the moment its own commit added a `.radix.tsx` twin for an unrelated reason) — fixed and amended in. Final commit `14c88cc`. Task 11 complete; branch is ready to merge.
