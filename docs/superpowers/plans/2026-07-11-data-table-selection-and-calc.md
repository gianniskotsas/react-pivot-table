...
All four re-verified: full suite (240/240), typecheck/lint/build clean, and live in the browser (shift-click + footer recalculation confirmed correct post-refactor; built `data-table-radix.json`'s content independently inspected via `node -e` to confirm it contains the Radix-specific code, not the base-ui version). A follow-up re-review of the fix itself (not yet dispatched at the time of this note — see next commit) is recommended before merge given the severity of what this review caught, matching the same discipline applied earlier in this plan for the `row.index` fix.
