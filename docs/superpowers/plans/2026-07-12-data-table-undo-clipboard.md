...
**Deviations, all found and disclosed by the implementer, all verified genuine:** the `DataTable`-level `onCreateRows` test's pasted text was changed from a 1-line to a 3-line block, since the fixture's 2 existing rows meant a 1-line paste would land as an UPDATE, never overflowing into `onCreateRows` at all — spec review independently confirmed the fixture really does have 2 rows and the original text really never would have triggered the callback; `firstCell.focus()`/`fireEvent.focus()` wrapped in `act(...)` to silence an act-warning (cosmetic); the `handleKeyDown` doc comment updated to stop referencing "later tasks" (comment-only, confirmed via diff that no logic lines changed). Spec review hand-traced the "Pasted N cells" toast math against `planPaste`'s real behavior and confirmed it's correctly suppressed (not just correctly counted) when every pasted cell fails to parse. Code-quality review found two real test-coverage gaps in exactly that "silently does nothing" territory — no test asserted the toast-suppression path for an all-unparseable paste, and no test asserted `clearSelectedOrActiveCells`'s full no-op path (neither a selection nor an active cell) — both added with the reviewer's exact specified tests. It also found `clearSelectedOrActiveCells` was missing the same selection-wins rationale comment `copy` already carries (this file's established convention is a comment on every callback explaining non-obvious tie-breaks); added, explicitly noting the selection-always-wins choice holds even when the active cell sits outside the selection, mirroring `copy`'s established precedent. Final: 44/44 `use-data-table.test.tsx`, 219/219 full suite, lint/typecheck clean. Commit `99eab0e` (amended).
...
**Deviation, disclosed and pre-flagged by the plan itself:** the plan's own suggested test used `vi.doMock`, which the plan explicitly warned might not work given this file's static top-of-file `import { DataTable } from "./data-table"` (hoisting resolves it before any `vi.doMock` inside a test body runs). The implementer used `import * as ExportCsvModule from "./export-csv"` + `vi.spyOn(ExportCsvModule, "downloadCsv")` instead — spec review independently confirmed this genuinely intercepts the real call via an actual mutation probe (temporarily making the spy throw, confirming the error's stack trace shows it fires from inside `data-table.tsx`'s real click handler, not a no-op). Code-quality review traced `getSortedRowModel()` through TanStack's actual row-model layering and confirmed it's pre-pagination (core → filtered → sorted, with `getPaginationRowModel` only slicing AFTER), so export correctly covers every sorted/filtered row across all client-side pages, not just the current page — and found a real test-coverage gap: the single test never exercised either of the two exclusion branches `handleExport` actually has logic for (a hidden column via the Columns menu, and the row-gutter column under `enableRowSelection`) — both added with the reviewer's exact specified tests, one using exact-equality on the header line specifically so a regression in the `ROW_GUTTER_COLUMN_ID` filter falling back to the raw `"__row-gutter__"` id would be caught, not silently passed by a looser substring check. Filename (`"export.csv"`) and the lack of try/catch around the synchronous `exportCsv`/`downloadCsv` calls were both confirmed as-designed (the plan's own literal given code; no realistic promise-rejection path to swallow, unlike copy/paste's async clipboard calls). Final: 16/16 `data-table.test.tsx`, 222/222 full suite, lint/typecheck clean. Commit `0f32028` (amended).
